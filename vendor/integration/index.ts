    ...
    hooks: {
      ...
      'astro:build:done': async ({ logger }) => {
        ...
        try {
          ...
          // The Cloudflare adapter emits static output under <outDir>/client/,
          // plain static builds under <outDir> — find the sitemap in either.
          const outDir =
            [cfg.outDir, new URL('client/', cfg.outDir)].find((d) => fs.existsSync(new URL(sitemapName, d))) ??
            cfg.outDir;
